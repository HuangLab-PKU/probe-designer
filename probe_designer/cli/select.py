...
def select(
    ...
    region_size: int = typer.Option(
        80, "--region-size",
        help="Cluster width in bp for round-robin selection. Larger = coarser "
             "clustering (fewer, bigger clusters; picks may crowd). Smaller = "
             "finer clustering (more picks forced apart). Match this to the "
             "peak_rank setting used upstream when possible.",
    ),
) -> None:
    """Select top-N highest-scoring sites per gene with round-robin-across-clusters spread."""
    data = json.loads(input_path.read_text(encoding="utf-8"))
    if not isinstance(data, dict):
        raise typer.BadParameter("Input JSON must be a {gene: [sites]} mapping.")

    selected_by_gene = {
        gene: select_top_n_with_gap(
            sites, top_n=top_n, min_gap=min_gap, region_size=region_size,
        )
    ...
    logger.info(
        "Selected %d of %d sites across %d genes (top_n=%d, min_gap=%d, region_size=%d).",
        total_out, total_in, len(selected_by_gene), top_n, min_gap, region_size,
    )
