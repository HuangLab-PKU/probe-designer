...
def position_search(sequence,  
    ...
    for record in plp_pos_candidate:
        if record['pos'] in pos_best:
            pos = record['pos']
            record['target_seq'] = sequence[pos : pos + BDS_len]
            record["plp_bds3'"] = seq_minus(record['target_seq'])[: BDS_len // 2]
            record["plp_bds5'"] = seq_minus(record['target_seq'])[BDS_len // 2 :]
...
def plp_wanted(record, plpargs):
    ...
    wanted = True
    # non consective base
    if "G" * G_consecutive in record['target_seq']: wanted = False
    # check G 40%-70%,
    G_pct = record['target_seq'].count("G") / len(record['target_seq'])
...
def plp_evaluation(record, plpargs):
    G_consecutive = plpargs.get("G_consecutive", 5)
    G_min = plpargs.get("G_min", 0.3)
    G_max = plpargs.get("G_max", 0.7)
    Tm_low = plpargs.get("Tm_low", 50)
    ...
    evaluation = {}
    # non consective base
    if "G" * G_consecutive in record['target_seq']: 
        evaluation['G_condition'] = False
        wanted = False
    # check G 40%-70%,
    G_pct = record['target_seq'].count("G") / len(record['target_seq'])
    if G_pct < G_min or G_pct > G_max: 
        evaluation['G_pct_conditon'] = False
        wanted = False
    # Tm too high or too low
    if record["plp_Tm3'"] > Tm_high or record["plp_Tm3'"] < Tm_low or record["plp_Tm5'"] > Tm_high or record["plp_Tm5'"] < Tm_low: 
        evaluation['Tm_conditon'] = False
        wanted = False 
    # Tm difference too great
    if abs(record["plp_Tm3'"] - record["plp_Tm5'"]) > Tm_dif_thre: 
        evaluation['Tm_thre_condition'] = False
        wanted = False 
    # check 2nd structure
    if record["mfe"] < mfe_thre: 
        evaluation['mfe_conditon'] = False
        wanted = False
