#!/usr/bin/env python3
"""DEPRECATED — use ``probe-design design --strategy isoform_specific``.
...
def _parse_legacy_args(argv: list[str]) -> list[str]:
    p = argparse.ArgumentParser(description="Legacy find_specific_probes shim")
    ...
    args = ["design", "--strategy", "isoform_specific"]
    if a.config:
...
def main() -> int:
    warnings.warn(
        "scripts/find_specific_probes.py is deprecated; use "
        "'probe-design design --strategy isoform_specific' instead. "
        "This shim will be removed in probe-designer 0.3.0.",
        DeprecationWarning,
        stacklevel=2,
    )
    print(
        "[deprecation] find_specific_probes.py: please migrate to "
        "'probe-design design --strategy isoform_specific'.",
        file=sys.stderr,
    )
